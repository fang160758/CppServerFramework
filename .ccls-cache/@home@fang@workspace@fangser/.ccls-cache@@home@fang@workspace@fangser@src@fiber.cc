...
Fiber::Fiber(std::function<void()> cb, 
    ...
    m_ctx.us_stack.ss_sp = m_stack;
    m_ctx.us_stack.ss_size = m_stacksize;
...
void Fiber::swapIn() {
    ...
    int ret = swapcontext(&Scheduler::GetMianFiber()->m_ctx, &m_ctx);
    if (ret != 0) {
...
void Fiber::swapOut() {
    ...
    SetThis(Scheduler::GetMianFiber()); //让出cpu给线程的主协程
    int ret = swapcontext(&m_ctx, &Scheduler::GetMianFiber()->m_ctx);
    if (ret != 0) {
        FANG_LOG_ERROR(g_logger) << "swapcontext error, and error="
                                << strerror(errno);
    }
