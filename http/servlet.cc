...
int32_t FunctionServlet::handle(fang::http::HttpRequest::ptr request
               , fang::http::HttpResponse::ptr response
               , fang::http::HttpSession::ptr session) {
    return m_cb(request, response, session);
}

ServletDispatch::ServletDispatch()
    :Servlet("ServletDispatch") {
    m_default.reset(new NotFoundServlet("fang/1.0"));
}

int32_t ServletDispatch::handle(fang::http::HttpRequest::ptr request
               , fang::http::HttpResponse::ptr response
               , fang::http::HttpSession::ptr session) {
...
int32_t NotFoundServlet::handle(fang::http::HttpRequest::ptr request
                   , fang::http::HttpResponse::ptr response
                   , fang::http::HttpSession::ptr session) {
    response->setStatus(fang::http::HttpStatus::NOT_FOUND);
    response->setHeader("Server", "fang/1.0.0");
