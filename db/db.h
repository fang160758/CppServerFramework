...
#ifndef __FANG_DB_DB_H__
#define __FANG_DB_DB_H__
...
class ISQData {
public:
    typedef std::shared_ptr<ISQData> ptr;
    virtual ~ISQData() {}
...
public:
    virtual ~ISQLQuery() {}
    virtual ISQData::ptr query(const char* format, ...) = 0;
    virtual ISQData::ptr query(const std::string& sql) = 0;
};
...
public:
    ...
    virtual IStmt::ptr perpare(const std::string& stmt) = 0;
    virtual int getError() = 0;
    virtual std::string getErrorStr() = 0;
