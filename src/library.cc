...
public:
    ...
    void operator()(Module* module) {
        ...
        if (rt) {
            FANG_LOG_ERROR(g_logger) << "dlclose handle fail handle="
                << m_handle << " name=" << name
                << " version=" << version
                << " path=" << path
                << " error=" << dlerror();
        } else {
            FANG_LOG_INFO(g_logger) << "destory module success handle="
                << m_handle << " name=" << name
                << " version=" << version
                << " path=" << path;
        }
...
Module::ptr Library::GetModule(const std::string& path) {
    ...
    if (!handle) {
        FANG_LOG_ERROR(g_logger) << "cannot load library path="
            << path << " error=" << dlerror();
       return nullptr; 
    }
    ...
    if (!create) {
        FANG_LOG_ERROR(g_logger) << "cannot load symbol CreateModule in "
            << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    if (!destory) {
        FANG_LOG_ERROR(g_logger) << "cannot load symbol DestoryModule in "
            << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    FANG_LOG_INFO(g_logger) << "load module name=" << module->getName()
        << " version=" << module->getVersion()
        << " path=" << module->getFilename()
        << " success";
    
    Config::LoadFromConfDir(fang::EnvMgr::GetInstance()->getConfigPath(), true);
    return module;
}
